...
int send_request_pack(int s, int type)
{
  ...
  while ((sendto(s, request_pack, 200, 0, &sendto_sin, sizeof(sendto_sin)) < 0)
          && attempt < 10) {
    attempt++;
    perror("sendto:");
  }
...
/* Process command line arguments and switches ----------------------- */
  ...
  while ((arg_c = getopt(argc, argv, "?a:p:oi:vr:")) != EOF) {
    switch(arg_c) {
     case ('?') : 
       fprintf(stderr, "USAGE: %s [-a audiofile | -o] [-p receiveport] [-r serverport] [-i receiver_ip_address]\n", argv[0]);
       return;
     case('a') :
...
/* open sockets for sending and receiving packets */
  ...
  bind_success = bind (sock1, &sin, sizeof(sin));
  ...
  bind_success = bind (sock2, &sin, sizeof(sin));
...
/* This is main loop ------------------------------------------------- */
  ...
  while(1)
  {
    ...
    len = sizeof(sin);
    n = recvfrom(sock2, buffer, BUFSZ, 0, &sin, &len);
    if (n < 0)
    {
