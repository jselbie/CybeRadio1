...
cr1: cr1.o $(AUDIO_OBJ) circ.o rate.o libst.o
	$(CC) cr1.o circ.o rate.o libst.o $(AUDIO_OBJ) -o cr1 $(LIBS) $(AUDIO_LIBS)
...
terminate: terminate.o
	$(CC) terminate.o -o terminate $(LIBS)

talk: talk.o
	$(CC) talk.o -o talk $(LIBS)

tarup:
	tar -cvf CyberRadio1-a.tar audio_sgi.c audio_sgi.h 	\
	bconsole.c cr1.c cr1.h cr1.py packet.format protocol 	\
	request.c server.c shm.c shm.h tables.c tables.h talk.c	\
	terminate.c wwwdemo.cgi wwwdemo.c libst.h libst.c 	\
	circ.h circ.c rate.h rate.c Makefile README COPYING 	\
	INSTALL MISC RUNNING HACKING
	gzip CyberRadio1-a.tar

clean:
	rm cr1 server request bconsole terminate talk *.o


cr1.o: cr1.h rate.h circ.h
circ.o: circ.h
rate.o: rate.h
libst.o: libst.h
